...
vcf_with_metadata %>%
  ...
  labs(title = "SNPs Found Across Organism Samples",
...
vcf_with_metadata %>%
  ...
  ggplot(aes(x = pos,
             ...
             color = tissue)) +
  geom_point(size = 5, alpha = 0.6) +
  labs(title = "SNPs Found in H. sapien SARS-CoV-2 Infected Tissues",
       x = "Position on Genome",
       y = "SNP Quality",
       color = "Tissue Type") +
  theme_few()
...
vcf_with_metadata %>%
  ...
  ggplot(aes(x = pos,
             ...
             color = cell_type)) +
  geom_point(size = 5, alpha = 0.6) +
  labs(title = "SNPs Found in C. aethiops SARS-CoV-2 Infected Cells",
       x = "Position on Genome",
       y = "SNP Quality",
       color = "Cell Line") +
  theme_few()
...
vcf_with_metadata %>%
  ...
  ggplot(aes(x = pos,
             ...
             color = treatment)) +
  geom_point(size = 5, alpha = 0.6) +
  labs(title = "SNP Count Based on Treatment of H. sapien cells",
       x = "Position on Genome",
       y = "SNP Quality",
...
```{r human-cells-plot}
# Scatterplot of SNP quality scores in H. sapiens per cell line
vcf_with_metadata %>%
  filter(organism == "Homo sapiens") %>%
  filter(qual > 200) %>%
  mutate(qual = as.numeric(qual), # converts qual to numeric values
  ...
  ggplot(aes(x = pos,
             ...
             color = cell_line)) +
  geom_point(size = 5, alpha = 0.6) +
  labs(title = "SNPs from H. sapien SARS-CoV-2 Infected Cell Lines",
       x = "Position on Genome",
       y = "SNP Quality",
       color = "Cell Line") +
  theme_few()
ggsave("output/human-cell-line_scatterplot.png", width = 8, height = 6)
```
...
vcf_with_metadata %>%
  ...
  ggplot(aes(x = pos,
             ...
             color = treatment)) +
  geom_point(size = 5, alpha = 0.6) +
  labs(title = "SNP Count Based on Treatment of C. aethiop cells",
       x = "Position on Genome",
       y = "SNP Quality",
       color = "Treatment Type") +
  theme_few()
...
vcf_with_metadata %>%
  ...
  knitr::kable(col.names = c("Gene Name",
                             "Number of SNPs"))
write.csv(vcf_with_metadata, "output/snp_tally.csv", row.names = FALSE)
```
...
vcf_with_metadata %>%
  ...
  knitr::kable()
write.csv(vcf_with_metadata, "output/snp_tally_data.csv", row.names = FALSE)
```
