...
SRA_RUNTABLE		:=	$(RUNTABLE_DIR)/2024-11-25_clean_data.csv
VCF_FOR_R_DIR		:=	data/11_vcf_output_for_R
VCF_FOR_R_FILES		:=	$(VCF_FOR_R_DIR)/$(wildcard *.vcf)

all: $(FLAGSTATS_FILES) $(FASTQC_FILES) output/Report.html

check:
	Rscript -e "lintr::lint_dir(pattern = rex::rex('.', or('R', 'r', 'Rmd', 'rmd'), end))"
	find . -name "*.sh" | xargs shellcheck

output/Report.html: Report.Rmd references.bib code/14_render_rmarkdown.sh $(R_FUNCTIONS) $(VCF_FOR_R_FILES) $(GENOME_REF_ANN) $(SRA_RUNTABLE)
